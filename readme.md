...
| [WooCommerce](docs/woocommerce.md) | Covers cache bypasses, customer sessions, WP-Cron and Action Scheduler, HPOS, application logs, email, payments, webhooks, staging safety, backups, headless APIs, and troubleshooting. |
...
| Module | Latest Version | What SlickStack manages |
| :------------- | :----------: | :---------- |
...
| **[Ubuntu LTS](docs/ubuntu.md)** | 24.04 | Ubuntu 24.04 is recommended for new servers; Ubuntu 22.04 remains supported, while 20.04 and 18.04 are legacy releases that require Ubuntu Pro or ESM for continued Ubuntu package maintenance. SlickStack manages package and kernel upgrades, required utilities, users, SSH, SFTP, sudoers, sysctl, swap, shell configuration, cron, and permissions. |
...
| **[PHP-FPM](docs/php-fpm.md)** | 8.3 | PHP 8.3 is recommended for new servers. PHP 8.1, 7.4, and 7.2 are retained for older accepted Ubuntu releases but are no longer supported by the PHP project. SlickStack manages packages, extensions, `php.ini`, `php-fpm.conf`, the shared `www` pool, OPcache, RAM-based tuning, logging, restarts, and service recovery. |
| **[MySQL](docs/mysql.md)** | 8.0 | Local or remote mode, package and service setup, `my.cnf`, InnoDB and RAM-based tuning, WordPress users and databases, dumps, imports, optimization, restarts, and local service recovery. |
