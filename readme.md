...
* **NEW!** SlickStack now supports fully customizing the MU (Must-Use) plugins that are installed. This can be done easily by definine the download link and desired directory name of each MU plugin within the `ss-config` advanced settings. If these variables are missing for any reason, SlickStack will default to the normal LittleBizzy plugins. Keep in mind that the Autoloader, Object Cache, Custom Functions, and XXX Notices plugins are required and cannot be disabled at this time.
