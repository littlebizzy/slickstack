...
&#10142; **New!** SlickStack now receives an "A" grade from [SecurityHeaders.com](https://securityheaders.com/?q=https%3A%2F%2Fwww.littlebizzy.com&followRedirects=on)...
...
| **PHP-FPM** | 7.2.15 | `php.ini` + `php-fpm.conf` + `www.conf` |
| **Zend/OPcache** | 3.2.0/7.2.15 | (same as above) |
...
| **UFW Firewall** | 0.35 | `ufw` + `ufw.conf` + `user-rules` |
...
* [XXX Notices](https://github.com/littlebizzy/slickstack/blob/master/mu-plugins/xxx-notices.txt): Variable notices designed to appear in the WP Admin Dashboard that mention important changes in regard to the SlickStack or LittleBizzy environment.
